...
-- load two small dataset directly ['pizzas' & 'pizza_details']
-- create table for 2 big daatasets ['orders' & 'order_details']
...
SELECT 
    SUM(order_details.quantity) Common_pizza, pizzas.size
FROM
    order_details
        JOIN
    pizzas ON order_details.pizza_id = pizzas.pizza_id
...
SELECT 
    ROUND(AVG(quantity), 0) AS avg_sellperday
FROM
    (SELECT 
...
group by orders.order_date) as total_sale
; 
...
group by category, name) as revenue_tbl) as rnk_tbl
where rnk < 4
;
